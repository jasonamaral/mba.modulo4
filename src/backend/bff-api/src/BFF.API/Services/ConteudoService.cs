    ...
    public class ConteudoService : BaseApiService, IConteudoService
    {
        ...
        public async Task<ResponseResult<CursoDto>> ObterCursoPorId(Guid cursoId, string token, bool includeAulas = false)
        {
            if (!ValidateId(cursoId, nameof(ObterCursoPorId)) || !ValidateToken(token, nameof(ObterCursoPorId)))
                return new ResponseResult<CursoDto> { Status = 400, Errors = new ResponseErrorMessages { Mensagens = new List<string> { "Parâmetros inválidos" } } };
            ...
            var result = await ExecuteWithErrorHandling(async () =>
            {
                ...
                return await _restApiService.GetAsync<ResponseResult<CursoDto>>(url, headers);
        ...
        public async Task<ResponseResult<PagedResult<CursoDto>>> ObterTodosCursos(string token, CursoFilter filter)
        {
            ...
            {
                var headers = CreateAuthHeaders(token);
                return await _restApiService.GetAsync<ResponseResult<PagedResult<CursoDto>>>("api/cursos", headers);
        ...
        public async Task<ResponseResult<Guid>> AdicionarCurso(CursoCriarRequest curso, string token)
        {
            ...
            {
                var headers = CreateAuthHeaders(token);
                return await _restApiService.PostAsync<CursoCriarRequest, ResponseResult<Guid>>("api/cursos", curso, headers);
        ...
        public async Task<ResponseResult<CursoDto>> AtualizarCurso(Guid id, AtualizarCursoRequest curso, string token)
        {
            ...
            {
                var headers = CreateAuthHeaders(token);
                return await _restApiService.PutAsync<AtualizarCursoRequest, ResponseResult<CursoDto>>($"api/cursos/{id}", curso, headers);
