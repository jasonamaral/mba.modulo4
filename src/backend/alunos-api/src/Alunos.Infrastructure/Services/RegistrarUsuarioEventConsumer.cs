...
public class RegistrarUsuarioEventConsumer : BackgroundService
{
    private readonly IServiceProvider _serviceProvider;
    private readonly ILogger<RegistrarUsuarioEventConsumer> _logger;
    private readonly IConfiguration _configuration;
    private IConnection? _connection;
    private IModel? _channel;

    public RegistrarUsuarioEventConsumer(
        IServiceProvider serviceProvider,
        ILogger<RegistrarUsuarioEventConsumer> logger,
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        try
        {
            ...
            consumer.Received += async (model, ea) =>
            {
                ...
                try
                {
                    var userRegisteredEvent = JsonConvert.DeserializeObject<RegistrarUsuarioEvent>(message);
                    
                    if (userRegisteredEvent != null && !userRegisteredEvent.EhAdministrador)
                    {
                        using var scope = _serviceProvider.CreateScope();
                        var handler = scope.ServiceProvider.GetRequiredService<RegistrarUsuarioEventHandler>();
